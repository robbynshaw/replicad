...
import * as ReplicadOC from "replicad-opencascadejs";
...
return main(replicad, __inputParams || dp)
  `;
    ...
    async function runAsModule(code: string, params: any) {
...
return dp
  `;
    ...
    let OC: Promise<ReplicadOC.OpenCascadeInstance> = initOpenCascade().then(
      (oc) => {
        return oc;
      }
    );
    ...
    const faceInfo = (
      ...
      const face =
        SHAPES_MEMORY[shapeId]?.[subshapeIndex]?.shape.faces[faceIndex];
      if (!face) return null;
      return {
        type: face.geomType,
    ...
    const edgeInfo = (
      ...
      const edge =
        SHAPES_MEMORY[shapeId]?.[subshapeIndex]?.shape.edges[edgeIndex];
      if (!edge) return null;
      return {
        type: edge.geomType,
